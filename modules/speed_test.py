...
class SpeedTestClient:
    ...
    def run_test(self, server_ip: str, port: int = DEFAULT_PORT,
        ...
        try:
            ...
            while True:
                ...
                if now - last_report >= 0.5:
                    ...
                    if progress_callback:
                        progress_callback(
                            f"Velocidad actual: {current_speed:.1f} Mbps",
                            min(pct, 0.99)
                        )
                    last_report = now
            ...
            client_speed_mbps = (total_bytes * 8) / (total_elapsed * 1_000_000) if total_elapsed > 0 else 0
            ...
            self._log(f"Test completado: {client_speed_mbps:.2f} Mbps ({client_speed_MBs:.2f} MB/s)")
            return result
