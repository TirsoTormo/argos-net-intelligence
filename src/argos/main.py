...
===============================================
Enterprise CLI tool for network engineering:
- Local network device discovery
- LAN speed test between devices
- Custom packet factory (Layers 2/3/4 of the OSI model)

Usage:
    python argos.py                    → Interactive menu
    python argos.py --scan             → Quick network scan
    python argos.py --interfaces       → Show interfaces
    python argos.py --server           → Speed test server
    python argos.py --client <IP>      → Speed test client
    python argos.py --dst <IP> --flags S --port 443   → Send custom TCP
    python argos.py --probe <IP> --ports web          → TCP port probe
    python argos.py --traceroute <IP>  → Manual traceroute

Argos — Enterprise-Grade Network Tool v1.0
Requires administrator privileges for Layer 2 operations and TCP flags.
"""
...
def auto_installer():
    ...
    if missing:
        ...
        try:
            ...
            print(
                "\033[31m  X Error: Could not install dependencies automatically.\033[0m"
            )
            print("\033[31m  Please run: pip install -r requirements.txt\033[0m")
            sys.exit(1)
    ...
    if not is_admin:
        ...
        console.print(
            Panel(
                ...
                "  [dim]On Linux:[/dim]    [magenta]Use sudo python main.py[/magenta]",
...
Examples:
  ...
  python main.py --server                 Speed test server
  python main.py --client 192.168.1.10    Speed test client
  ...
  python main.py --ping 192.168.1.1 --count 10 --ttl 128
    ...
    pf.add_argument(
        "--size", type=int, default=56, help="ICMP payload size in bytes (default: 56)"
    )
...
# ─────────────────────────────────────────────────────────────
# Direct Commands (Non-Interactive Mode)
# ─────────────────────────────────────────────────────────────
...
def cmd_quick_scan(args=None):
    ...
    from argos.ui.report import display_animated_device_table, create_scan_summary
    ...
    if devices:
        ...
        console.print("[dim]Optimizing: Identifying Vendors (Active Concurrency)...[/dim]")
        vm = VendorManager()
        ...
        if db.save_scan(scan_result):
            console.print("[dim green]Scan saved to local database (argos_audit.db).[/dim green]")

        if args:
            from argos.storage.exporter import ReportExporter
            if args.export_json:
                if ReportExporter.to_json(args.export_json, scan_result):
                    console.print(f"[green]JSON report saved at: {args.export_json}[/green]")
            if args.export_md:
                if ReportExporter.to_markdown(args.export_md, scan_result):
                    console.print(f"[green]MD report saved at: {args.export_md}[/green]")
            if args.export_csv:
                if ReportExporter.to_csv(args.export_csv, devices):
                    console.print(f"[green]CSV report saved at: {args.export_csv}[/green]")
...
def cmd_show_interfaces():
    ...
    from argos.core.net_utils import get_local_interfaces
    from argos.ui.report import create_interface_table
...
def cmd_server(port: int = 45678):
    ...
    from argos.ui.report import create_speed_result_panel
...
def cmd_client(server_ip: str, port: int, duration: int):
    ...
    if not is_private_ip(server_ip):
        console.print(f"[bold red]✗ {server_ip} is not a private IP. Aborted.[/bold red]")
        return
    ...
    if result:
        ...
        console.print("[bold red]✗ Could not complete the test.[/bold red]")


def cmd_tcp_custom(dst_ip: str, port: int, flags: str, src_port=None):
    """Sends a custom TCP segment."""
    from argos.core.packet_factory import send_tcp_custom, describe_flags
...
def cmd_tcp_probe(dst_ip: str, ports_input: str):
    ...
    console.print(
        f"\n  [bold bright_green]Open ports: {len(open_ports)}"
...
def main():
    ...
    # Detect if any direct mode was requested
    if args.scan:
        cmd_quick_scan(args)
        ...
        # Interactive mode
        from argos.ui.cli_ui import main_loop
