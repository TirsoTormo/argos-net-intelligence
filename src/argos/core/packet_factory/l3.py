...
def send_icmp_ping(target_ip: str, count: int = 4, ttl: int = 64, payload_size: int = 56, log_callback: Optional[Callable] = None) -> Dict:
    """Sends a custom ICMP Ping."""
    _validate_target(target_ip)
    _log_msg(log_callback, f"[LAYER 3] ICMP Echo Request → {target_ip} (TTL: {ttl})")
    
    conf.verb = 0
    stats = {"dst": target_ip, "sent": 0, "received": 0, "lost": 0, "min_ms": None, "max_ms": None, "avg_ms": 0}
    latencies = []
    ...
    for i in range(count):
        ...
        try:
            ...
            if ans:
                ...
                _log_msg(log_callback, f"  Reply from {target_ip}: bytes={payload_size} latency={lat}ms TTL={ans.ttl}")
            else:
...
def manual_traceroute(target_ip: str, max_hops: int = 30, log_callback: Optional[Callable] = None) -> List[Dict]:
    ...
    for ttl in range(1, max_hops + 1):
        ...
        try:
            ...
            if ans is None:
                ...
                hops.append({"ttl": ttl, "ip": ans.src, "latency_ms": elapsed, "status": "ok", "response_ttl": ans.ttl})
